# GitHub Repository Setup & Secret Management

## Current Steps Taken

### 1. Initialize Git Repository

![Initialize Git](InitializeGit.png)

### 2. Configure Git Using the Terminal

![Terminal](Terminal.png)

### 3. Pull the Repository

![Pull Repository](Pull.png)

---

## 08/28

## 4. Opening the Collection in the Terminal

![Open Collection Terminal](OpenCollectionTerminal.png)

## 5. Opening a GitHub Repository

![Open GitHub Repository](OpenGitHubRepo.png)

## 6. Open My Repository

![Open My Repository](OpenMyRepo.png)

## 7. Import a Private Repository

![Import Private Repository](ImportPrivate.png)

## 8. Verify Secret Key Handling

The secret key was verified to ensure that it was not stored directly in the repository.

![Pulls Blank Secret Key](PullsBlankSecretKey.png)

## 9. Configurable External Secrets Manager

Does not specifically have Delinea built in.  Bruno supports bru.runRequest(), runtime variables, environment variables, and scripting APIs for manipulating requests.  So, potentially, Bruno's scripting API is flexible enough to build a Delinea integration at the collection level.

![External Secrets Manager](ExternalSecretsManager.png)

## 10. OpenAPI

![Open API](OpenAPI.png)
